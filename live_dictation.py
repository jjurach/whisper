...
def stop_recording_and_process():
    ...
    if is_recording:
        ...
        print(f"Control key held down for: {key_press_duration:.2f} seconds", flush=True)
        ...
        recorded_duration = len(raw_audio_buffer) / (RATE * p.get_sample_size(FORMAT) * CHANNELS)
        print(f"Captured {len(audio_frames)} frames, totaling {recorded_duration:.2f}s.", flush=True)
        ...
        audio_data = np.frombuffer(raw_audio_buffer, dtype=np.int16).astype(np.float32) / 32768.0

        # 4. Resample for Whisper
        if RATE != 16000:
            num_samples = round(len(audio_data) * 16000 / RATE)
            audio_data = resample(audio_data, num_samples)
            print(f"Resampled audio to {len(audio_data)} samples for Whisper.", flush=True)
...
def main():
    ...
    try:
        ...
        stream = p.open(format=FORMAT,
                        ...
                        start=False) # Open stream in 'stopped' state
    ...
    try:
        ...
        print(f"Press **ESCAPE** to quit.", flush=True)
        if args.timeout:
            print(f"Script will automatically exit after {args.timeout} seconds.", flush=True)
        print("-" * 40, flush=True)
