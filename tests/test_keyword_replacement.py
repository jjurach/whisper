...
from live_whisper.live_dictation import process_text_with_llm  # noqa: E402
...
class TestKeywordReplacement(unittest.TestCase):
    ...
    def test_basic_punctuation(self):
        self.assertEqual(
            process_text_with_llm("This is a test period"), "This is a test."
        )
        self.assertEqual(
            process_text_with_llm("Apples COMMA oranges"), "Apples,oranges"
        )
        self.assertEqual(process_text_with_llm("How much dollar sign"), "How much$")

    def test_multi_word_commands(self):
        self.assertEqual(
            process_text_with_llm("What time is it question mark"), "What time is it?"
        )
    ...
    def test_no_false_matches(self):
        ...
        # Hashtag is a keyword, so it SHOULD be replaced
        self.assertEqual(process_text_with_llm("Hashtag rocks"), "# rocks")
    ...
    def test_symbols_and_special_chars(self):
        # Implementation removes space before $
        self.assertEqual(
            process_text_with_llm("Cost is fifty dollar sign"), "Cost is fifty$"
        )
        self.assertEqual(process_text_with_llm("Use hashtag"), "Use #")

    def test_quotes_and_parentheses(self):
        self.assertEqual(
            process_text_with_llm("He said open quote hello close quote"),
            'He said "hello"',
        )
        self.assertEqual(
            process_text_with_llm(
                "Use open parenthesis custom close parenthesis keyword"
            ),
            "Use (custom) keyword",
        )
    ...
    def test_colon_and_semicolon(self):
        ...
        self.assertEqual(
            process_text_with_llm("Item one semicolon item two"), "Item one; item two"
        )
