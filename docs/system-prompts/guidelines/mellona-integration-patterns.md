# Mellona Integration Patterns: Observed Failure Modes & Self-Healing Guidance

**Purpose:** Document real failure patterns encountered during mellona integration across multiple modules (second_voice, pigeon, hatchery) and provide targeted guidance to prevent recurrence.

**Basis:** Synthesized from Phase 1-8 mellona development and integration documentation (hentown-aog, hentown-ltc, hentown-a7y, hentown-07r).
...
**Pattern:** Multiple modules initialize `mellona.get_config()` with different config chains, leading to unpredictable credential resolution and provider availability.

**Example from second_voice Phase 3:**
```python
# Module A initializes global config
config = get_config(config_chain=['/path/to/moduleA/config.yaml', mellona_config_path])
...
- Include fallback paths explicitly: app-specific settings FIRST, then shared mellona config
...
if not hasattr(AIProcessor, '_mellona_configured'):
    get_config(config_chain=[
        '~/.config/second_voice/settings.json',  # App-specific first
        '~/.config/mellona/config.yaml'          # Fallback to shared mellona
    ])
    AIProcessor._mellona_configured = True

# Now SyncMellonaClient uses this global config
self.client = SyncMellonaClient()
```
...
**Pattern:** Mellona providers return `None` when credentials are missing rather than raising exceptions. Consuming code doesn't check for `None`, assuming provider initialization always succeeds.

**Example from pigeon professionalize:**
```python
...
- Use `provider.is_available()` property before attempting transcription
...
**Pattern:** Tests use `requests_mock` to mock raw HTTP calls. When code is refactored to use mellona (which abstracts HTTP), the mocks don't intercept, tests fail with actual API calls or 404s.

**Example from second_voice tests:**
```python
# Old test (working)
...
# After mellona integration, same test fails because:
# - mellona wraps the HTTP call
# - requests_mock sees SyncMellonaClient's HTTP call, not the raw one
# - Mock expectations don't match mellona's exact request format
```

**Impact:** 9 out of 232 second_voice tests fail after mellona integration. Test suite doesn't catch regression during integration work.

### Self-Healing Guidance

**✅ DO:**
- After integrating mellona providers, update mocks to target mellona's HTTP layer
- Create fixture factories for mellona providers (mock implementations)
...
def mock_local_whisper_provider():
    ...
    from mellona import STTResponse
...
def test_transcribe_with_mock(mock_local_whisper_provider, monkeypatch):
    ...
    monkeypatch.setattr('mellona.get_provider',
...
**Pattern:** Modules don't understand mellona's credential resolution priority. They expect credentials in a specific file location, but mellona checks ENV > keyring > config file. Code passes one credential but mellona uses a different one from the chain.
...
3. If not found, check ~/.config/mellona/config.yaml
...
**✅ DO:**
- Document mellona's credential priority chain in every module that uses it
- Provide clear setup instructions: "Set GROQ_API_KEY or add credentials to ~/.config/mellona/config.yaml"
...
"""
Mellona Provider Credentials

mellona resolves credentials in this priority order:
1. Environment variable (e.g., GROQ_API_KEY, LOCAL_WHISPER_URL)
2. System keyring (if configured)
3. Config file (~/.config/mellona/config.yaml)
...
To verify credentials are detected:
  python -m mellona health-check
"""

# In code
from mellona import ProviderManager
...
if provider and provider.is_available():
    ...
    logger.warning("Groq provider not available - check GROQ_API_KEY env var or ~/.config/mellona/config.yaml")
```
...
**Pattern:** When mellona or other agents are spawned as subprocesses (hatchery executing Claude Code), timeout handling varies. 5-minute timeout may be too short for complex tasks, or too long for quick checks.

**Example from hatchery Epic 2:**
```python
...
**Impact:** Long-running mellona tasks (batch transcriptions, multiple provider health checks) timeout unexpectedly.
...
- For mellona providers: implement per-operation timeouts, not global timeouts
...
When integrating mellona into a new module, verify:
...
- [ ] Test suite updated: old HTTP mocks replaced with mellona provider mocks
...
### second_voice Phase 3 (STT Integration)
...
**Next:** Update tests to mock mellona providers instead of HTTP

### pigeon professionalize.py
- ✅ Uses mellona imports
...
### hatchery Epic 2 (Provider Abstraction)
...
**Basis:** Synthesis of mellona Phases 1-8 change documentation and real integration patterns
**Status:** Active ✓ — Guides future mellona integrations across all modules
